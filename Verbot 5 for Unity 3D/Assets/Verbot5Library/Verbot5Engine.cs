...
namespace Conversive.Verbot5
{
	...
	public class Verbot5Engine
	{
		...
		public Reply GetReply(string input, State state)
		{
            ...
            //first look for valid child rules to fire any any kb
            foreach (string stPath in state.CurrentKBs)
            {
                if (this.compiledKnowledgeBases.ContainsKey(stPath))
                {
                    ...
                    state.Lastinput = input;
                    Reply reply = ckb.GetChildReply(input, state.Lastfired, state.Vars, ckbOutput);
                    if (reply != null)
                    {
	...
	public class CompiledKnowledgeBase : ISerializable
	{
        ...
        public Reply GetChildReply(string input, string lastfired, Hashtable vars, string prependOutput)
        {
            ...
            if (this.inputs.ContainsKey(lastfired))
            {
                ...
                matches.AddRange(getChildReplyWithReplacedInput(input, inputReplaced, lastfired, vars));
            }

            return matchListToReply(matches, vars, prependOutput);
        }

        private List<Match> getChildReplyWithReplacedInput(string input, string inputReplaced, string lastfired, Hashtable vars)
        {
            List<Match> matches = new List<Match>();
        ...
        private Reply matchListToReply(List<Match> matches, Hashtable vars, string prependOutput)
        {
			if(matches.Count == 0)
				return null;
